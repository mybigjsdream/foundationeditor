/**
 * Created by dengjing on 16/1/26.
 */

// 编辑器部分的 路由
FlowRouter.route('/', {
    action: () => {
        console.log("Yeah! We are on the flowrouter.");
        BlazeLayout.render('layout');
    }
});


//blog 部分的 路由
FlowRouter.route('/blog', {
    //subscriptions: () => {
    //    FlowRouter.register('allblog', Meteor.subscribe('publish_article'));
    //},
    action: () => {
        BlazeLayout.render('blog_layout', {blog_content: 'blog_list' }); //不知道这样写是不是react时不好重构
    }
});
FlowRouter.route('/category/:category', {
    action: () => {
        BlazeLayout.render('blog_layout', {blog_content: 'blog_category_list' });
    }
});
FlowRouter.route('/author/:author', {
    action: () => {
        BlazeLayout.render('blog_layout', {blog_content: 'blog_author_list' });
    }
});

FlowRouter.route('/blog/:year/:month/:day/:title', {
    action: () => {
        //console.log('welcome '+params.id);
        BlazeLayout.render('blog_layout', {blog_content: 'blog_content'});
    }
});

FlowRouter.notFound = {
    subscriptions: () => {},
    action: () => {
        console.log('you are missing!');
        BlazeLayout.render('notFound');
    }
};